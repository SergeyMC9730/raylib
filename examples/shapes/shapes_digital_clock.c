...
int main(void)
{
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        BeginDrawing();
            ...
            RlDrawText("Press [SPACE] to switch clock mode", 10, 10, 20, DARKGRAY);
...
static void DrawClock(Clock clock, Vector2 centerPosition)
{
    if (clock.mode == MODE_HANDS_FREE)
    {
        ...
        RlDrawText(TextFormat("%i", clock.second.value), centerPosition.x + (clock.second.length - 10)*cosf(clock.second.angle*(float)(PI/180)) - DIGIT_SIZE/2, centerPosition.y + clock.second.length*sinf(clock.second.angle*(float)(PI/180)) - DIGIT_SIZE/2, DIGIT_SIZE, GRAY);

        RlDrawText(TextFormat("%i", clock.minute.value), clock.minute.origin.x + clock.minute.length*cosf(clock.minute.angle*(float)(PI/180)) - DIGIT_SIZE/2, centerPosition.y + clock.minute.length*sinf(clock.minute.angle*(float)(PI/180)) - DIGIT_SIZE/2, DIGIT_SIZE, RED);

        RlDrawText(TextFormat("%i", clock.hour.value), centerPosition.x + clock.hour.length*cosf(clock.hour.angle*(float)(PI/180)) - DIGIT_SIZE/2, centerPosition.y + clock.hour.length*sinf(clock.hour.angle*(float)(PI/180)) - DIGIT_SIZE/2, DIGIT_SIZE, GOLD);
    }
    else if (clock.mode == MODE_NORMAL)
    {
        // Draw hand seconds
        DrawRectanglePro((RLRectangle){ centerPosition.x, centerPosition.y, clock.second.length, clock.second.thickness },
            (Vector2){ 0.0f, clock.second.thickness/2.0f }, clock.second.angle, clock.second.color);

        // Draw hand minutes
        DrawRectanglePro((RLRectangle){ centerPosition.x, centerPosition.y, clock.minute.length, clock.minute.thickness },
            (Vector2){ 0.0f, clock.minute.thickness/2.0f }, clock.minute.angle, clock.minute.color);

        // Draw hand hours
        DrawRectanglePro((RLRectangle){ centerPosition.x, centerPosition.y, clock.hour.length, clock.hour.thickness },
