        ...
        BeginDrawing();
            ...
            if (splineHelpersActive)
            {
                ...
                for (int i = 0; i < pointCount; i++)
                {
                    ...
                    RlDrawText(TextFormat("[%.0f, %.0f]", points[i].x, points[i].y), (int)points[i].x, (int)points[i].y + 10, 10, BLACK);
                }
            }
            ...
            // Draw spline config
            GuiLabel((RLRectangle){ 12, 62, 140, 24 }, TextFormat("Spline thickness: %i", (int)splineThickness));
