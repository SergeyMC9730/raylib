...
int main(void)
{
    ...
    */
    ...
    while (!WindowShouldClose())    // Detect window close button or ESC key
    {
        ...
        if (IsWindowState(FLAG_BORDERLESS_WINDOWED_MODE)) RlDrawText("[B] FLAG_BORDERLESS_WINDOWED_MODE: on", 10, 280, 10, LIME);
        ...
        if (IsWindowState(FLAG_WINDOW_HIGHDPI)) RlDrawText("FLAG_WINDOW_HIGHDPI: on", 10, 340, 10, LIME);
        else RlDrawText("FLAG_WINDOW_HIGHDPI: off", 10, 340, 10, MAROON);
        if (IsWindowState(FLAG_WINDOW_TRANSPARENT)) RlDrawText("FLAG_WINDOW_TRANSPARENT: on", 10, 360, 10, LIME);
        else RlDrawText("FLAG_WINDOW_TRANSPARENT: off", 10, 360, 10, MAROON);
        if (IsWindowState(FLAG_MSAA_4X_HINT)) RlDrawText("FLAG_MSAA_4X_HINT: on", 10, 380, 10, LIME);
